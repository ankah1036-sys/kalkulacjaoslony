...
// ofertą; właściwą wysyłkę użytkowniczka kończy w swojej skrzynce.
export default function SentDialog({ open, to, offerNo, onClose }) {
  if (!open) return null;
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 70, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      ...
      <div style={{ position: "relative", width: "min(440px, 96vw)", background: C.paper, border: `2px solid ${C.ink}`, boxShadow: "0 20px 60px rgba(0,0,0,0.35)", textAlign: "center", padding: "28px 24px" }}>
        ...
        <div style={{ fontSize: 20, fontWeight: 800, letterSpacing: -0.3, marginBottom: 8 }}>Oferta przekazana do wysłania</div>
        <div style={{ fontSize: 14, color: C.steel, lineHeight: 1.6, marginBottom: 20 }}>
          Otworzył się Twój program pocztowy z gotową ofertą{offerNo ? ` ${offerNo}` : ""}
          {to ? (
            <>
